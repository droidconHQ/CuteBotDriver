...
* **2x AAA Batteries** or a rechargeable lithium pack for power
...
The micro:bit hosts a standard Nordic UART Service. Commands are sent from the mobile app as plain text strings written to the **RX Characteristic**. Every command must be terminated by a newline character (`\n`).
...
| Command | Description | Example Payload |
| :--- | :--- | :--- |
| `HL,[R],[G],[B]` | Set front left and right headlights | `HL,255,0,0\n` (Solid Red) |
| `HO` | Turn off all headlights and underglow | `HO\n` |
| `UG,[R],[G],[B]` | Set both bottom underglow NeoPixels | `UG,0,255,0\n` (Solid Green) |

### 3. Audio Commands

| Command | Description | Example Payload |
| :--- | :--- | :--- |
| `BEEP` | Play a short car-horn honk | `BEEP\n` |
| `SIREN` | Play a brief two-tone emergency siren loop | `SIREN\n` |

### 4. Sensor Feedback (Two-Way Requests)
...
* **Distance Query:** Send `?DIST\n`
  * The micro:bit responds with: `DIST:[value]\n` (where value is an integer string in centimeters)
